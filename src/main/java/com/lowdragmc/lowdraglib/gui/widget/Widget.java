...
public class Widget {
    ...
    public Widget setHoverTooltips(String... tooltipText) {
        tooltipTexts.clear();
        Arrays.stream(tooltipText).filter(Objects::nonNull).filter(s->!s.isEmpty()).map(Component::nullToEmpty).forEach(tooltipTexts::add);
        return this;
    }
    ...
    public void drawOverlay(@Nonnull PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        if (overlay != null) {
            Position pos = getPosition();
